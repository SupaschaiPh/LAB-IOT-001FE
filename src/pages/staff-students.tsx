...
export default function StaffStudentPage() {
  ...
  async function editHandler(id: number, values: any) {
    try {
      ...
      await axios.patch(`/students/${id}`, values);
      notifications.show({
        title: "แก้ไขข้อมูลนักเรียนสำเร็จ",
        message: "ข้อมูลนักเรียนได้รับการแก้ไขเรียบร้อยแล้ว",
        color: "teal",
      });
    } catch (error) {
      ...
      <section className="container mx-auto h-[600px] mt-4">
        ...
        <div className="flex w-full mb-2 items-center gap-2">
          ...
          <Button
            className="w-fit"
            variant="default"
            onClick={() => mutate(students, { revalidate: true })}
            loading={isValidating || isLoading}
          >
            รีเฟรชตาราง
          </Button>
          <Button component={Link} to="/students/create">
            เพิ่มนักเรียน
          </Button>
          <Button
            onClick={() => {
              modals.openConfirmModal({
                title: "คุณต้องการลบนักเรียนเล่มนี้ใช่หรือไม่",
                children: (
                  <span className="text-xs">
                    เมื่อคุณดำนเนินการลบนักเรียนเล่มนี้แล้ว
